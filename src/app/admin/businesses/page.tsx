      ...
      <div className="flex items-center justify-between">
        ...
        <div className="hidden sm:flex gap-2">
          <input
            ...
            className="h-10 w-72 rounded-xl bg-white/80 border border-black/10 px-3 outline-none focus:ring-2 focus:ring-caramel/40 shadow-[0_10px_30px_rgba(0,0,0,0.06)]"
          />
          <select
            value={plan}
            onChange={e =>
          ...
          {filtered.map(b => (
            <tr key={b.id} className="hover:bg-cream/60">
              <Td className="font-medium">
                <Link href={`/admin/businesses/${b.id}`} className="underline underline-offset-2">
                  {b.name}
                </Link>
              </Td>
              <Td>{b.ownerName}</Td>
              ...
              <Td>
                <Link href={`/admin/businesses/${b.id}`} className="chip bg-cream border border-black/10">
                  View
                </Link>
              </Td>
            </tr>
